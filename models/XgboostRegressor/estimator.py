...
from data_loader.data_preprocessing import (
    DataSet, one_hot_encoding, scaler, stratifiedKfold_regr
)
...
class XGBoostEstimator(Estimator):
    ...
    def fit(
        ...
        if validation_method == 'loocv':
            ...
            self.grid = GridSearchCV(
                xgb.XGBRegressor(), **hyperparam, scoring='neg_mean_squared_error', 
                cv=LeaveOneOut()
            ) 
        else:
            if stratified:
    ...
    def get_feature_importance(self, dataset_enc: pd.DataFrame, 
        ...
        if shap_:
            shap_results = shap_vals(
                self, dataset_enc.X_test, output_path, it, plot_, per_subset
            )
    ...
    def load(cls, path: str | Path, it: int=0) -> XGBoostEstimator:
        ...
        estimator = XGBoostEstimator(path)
