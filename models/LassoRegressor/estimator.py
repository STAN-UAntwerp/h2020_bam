...
class LassoEstimator(Estimator):
    ...
    def preprocess(self, dataset: DataSet) -> DataSet:
        ...
        # Lasso regressor is trained with CV, so we can combine train and validation set
    ...
    def fit(
        ...
        if validation_method == 'loocv':
            ...
            self.grid = GridSearchCV(
                Lasso(), **hyperparam, scoring='neg_mean_squared_error', 
                cv=LeaveOneOut()
            ) 
        else:
            if stratified:
                ...
                folds = stratifiedKfold_regr(
                    pd.concat([
                        dataset.X_train, dataset.y_train.to_frame(name=data_config.target)
                    ], axis=1),
        ...
        if len(i) > 0:
            ...
            if set=='test':
                ...
                y_pred = pd.Series(self.model.predict(dataset.X_train), index=dataset.X_train.index,
                                        name='predicted_' + data_config.target)
        return get_evaluation_metrics_reg(y, y_pred).rename('lasso')


    def plot_prediction(self, dataset: DataSet, df: pd.DataFrame, it: int=0):
    ...
    @classmethod
    def load(cls, path: str | Path, it: int=0) -> LassoEstimator:
